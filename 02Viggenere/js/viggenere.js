...
function codificar(texto, clave) {
    ...
    for (var i = 0; i < charArTexto.length; i++) {
        ...
        } else if (/^\d$/.test(charTexto)) {
            //numero
            resultado += (parseInt(charTexto) + des) % 10;
        } else {
            resultado += charTexto;
        }
...
function decodificar(texto, clave) {
    ...
    for (var i = 0; i < charArTexto.length; i++) {
        ...
        if (/^[a-zA-Z]$/.test(charTexto)) {
            // Si es una letra, aplica el cifrado Vigenere
            resultado += viggenere.decode(charTexto, (des >= 26) ? des % 26 : des);
        } else if (/^\d$/.test(charTexto)) {
            // Si es un número, aplica el cifrado usando módulo 10
            resultado += (parseInt(charTexto) - des + 10) % 10;
        } else {
            resultado += charTexto;
        }
...
function colocar() {
    var campoDeTexto = document.getElementById("res");
    ...
    campoDeTexto.setSelectionRange(0, 0);
